#!/bin/bash
# ============================================================================
# Compare two coarse topic-embedding sources for hierarchical QMSum routing.
#
# Purpose:
#   Verify whether we can replace the old "scan all turns online" topic scoring
#   with a cheaper precomputed topic representation, while preserving routing
#   quality and answer quality.
#
# Cases:
#   1. topic_embedding_source = online_turns
#   2. topic_embedding_source = precomputed_topic_text
#
# Usage:
#   cd ~/working_place/topic_selective_kv
#   bash scripts_qmsum/run_qmsum_topic_embedding_source_compare.sh
#
# Optional overrides:
#   START_DOC=5 END_DOC=6 MAX_QUERIES=1 ROUTE_TOP_K=16 \
#   TOPIC_PROTOTYPE_TURNS=3 ANSWER_MAX_NEW_TOKENS=96 \
#   bash scripts_qmsum/run_qmsum_topic_embedding_source_compare.sh
#
# Outputs:
#   logs/qmsum_topic_embedding_source_compare/*.log
#   logs/qmsum_topic_embedding_source_compare/summary.tsv
#   logs/qmsum_topic_embedding_source_compare/summary.txt
# ============================================================================
...
TOPIC_PROTOTYPE_TURNS=${TOPIC_PROTOTYPE_TURNS:-3}
ANSWER_MAX_NEW_TOKENS=${ANSWER_MAX_NEW_TOKENS:-96}
EVAL_ANSWERS=${EVAL_ANSWERS:-1}
PARALLEL_RUN=${PARALLEL_RUN:-1}
GPU_ONLINE_TURNS=${GPU_ONLINE_TURNS:-1}
GPU_PRECOMPUTED=${GPU_PRECOMPUTED:-2}
LOG_DIR=${LOG_DIR:-logs/qmsum_topic_embedding_source_compare}
...
echo "============================================================"
echo " QMSum Topic-Embedding Source Compare"
echo " model=$MODEL_ID"
echo " data=$DATA_PATH"
...
echo " gpu_online_turns=$GPU_ONLINE_TURNS"
echo " gpu_precomputed=$GPU_PRECOMPUTED"
echo " logs=$LOG_DIR"
echo "============================================================"

cat > "$SUMMARY_TSV" <<EOF
source	status	embedding_top1_pct	embedding_top2_pct	selected_topic_hit_pct	selected_turn_hit_pct	avg_turn_recall_pct	avg_turn_precision_pct	avg_turn_f1_pct	full_answer_f1_pct	selected_answer_f1_pct	answer_f1_delta_pct	ctx_token_saving_pct
EOF
...
run_case() {
    ...
    echo ""
    echo "============================================================"
    echo " Running source: $source_name"
    echo " GPU: $gpu_id"
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" python qmsum_sim.py \
        ...
        --topic_embedding_source "$source_name" \
    ...
    if [ "$pipe_status" -ne 0 ]; then
        echo " Source failed with exit code: $pipe_status" | tee -a "$log_file"
        append_summary "$source_name" "RUN_FAIL" "$log_file"
        return
    fi

    append_summary "$source_name" "OK" "$log_file"
}

if [ "$PARALLEL_RUN" -eq 1 ]; then
    run_case "online_turns" "$GPU_ONLINE_TURNS" &
    pid_online=$!
    run_case "precomputed_topic_text" "$GPU_PRECOMPUTED" &
    pid_precomputed=$!
    wait "$pid_online"
    wait "$pid_precomputed"
else
    run_case "online_turns" "$GPU_ONLINE_TURNS"
    run_case "precomputed_topic_text" "$GPU_PRECOMPUTED"
fi
...
{
    echo "============================================================"
    echo " QMSum Topic-Embedding Source Compare Summary"
    echo "============================================================"
