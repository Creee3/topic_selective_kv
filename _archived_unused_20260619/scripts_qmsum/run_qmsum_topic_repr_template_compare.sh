#!/bin/bash
# ============================================================================
# Compare topic representation templates for precomputed topic-text routing.
#
# Purpose:
#   Keep the same precomputed topic-text pipeline, but compare whether a
#   stronger topic representation template helps fix the top-1 ordering issue.
#
# Cases:
#   1. topic_representation_template = basic
#   2. topic_representation_template = enhanced
#
# Usage:
#   cd ~/working_place/topic_selective_kv
#   bash scripts_qmsum/run_qmsum_topic_repr_template_compare.sh
#
# Optional overrides:
#   START_DOC=5 END_DOC=6 MAX_QUERIES=1 HIER_TOP_TOPICS=1 \
#   TOPIC_PROTOTYPE_TURNS=5 GPU_BASIC=1 GPU_ENHANCED=2 \
#   bash scripts_qmsum/run_qmsum_topic_repr_template_compare.sh
#
# Outputs:
#   logs/qmsum_topic_repr_template_compare/*.log
#   logs/qmsum_topic_repr_template_compare/summary.tsv
#   logs/qmsum_topic_repr_template_compare/summary.txt
# ============================================================================
...
GPU_BASIC=${GPU_BASIC:-1}
GPU_ENHANCED=${GPU_ENHANCED:-2}
PARALLEL_RUN=${PARALLEL_RUN:-1}
LOG_DIR=${LOG_DIR:-logs/qmsum_topic_repr_template_compare}
...
echo "============================================================"
echo " QMSum Topic-Representation Template Compare"
echo " model=$MODEL_ID"
echo " data=$DATA_PATH"
...
echo " gpu_basic=$GPU_BASIC"
echo " gpu_enhanced=$GPU_ENHANCED"
echo " parallel_run=$PARALLEL_RUN"
echo " logs=$LOG_DIR"
echo "============================================================"

cat > "$SUMMARY_TSV" <<EOF
template	status	embedding_top1_pct	embedding_top2_pct	selected_topic_hit_pct	selected_turn_hit_pct	avg_turn_recall_pct	avg_turn_precision_pct	avg_turn_f1_pct	full_answer_f1_pct	selected_answer_f1_pct	answer_f1_delta_pct	ctx_token_saving_pct
EOF
...
run_case() {
    local template_name="$1"
    local gpu_id="$2"
    local log_file="$LOG_DIR/${template_name}.log"
    local extra_args=()
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" python qmsum_sim.py \
        ...
        --topic_representation_template "$template_name" \
    ...
    if [ "$pipe_status" -ne 0 ]; then
        echo " Template failed with exit code: $pipe_status" | tee -a "$log_file"
        append_summary "$template_name" "RUN_FAIL" "$log_file"
        return
    fi

    append_summary "$template_name" "OK" "$log_file"
}

if [ "$PARALLEL_RUN" -eq 1 ]; then
    run_case "basic" "$GPU_BASIC" &
    pid_basic=$!
    run_case "enhanced" "$GPU_ENHANCED" &
    pid_enhanced=$!
    wait "$pid_basic"
    wait "$pid_enhanced"
else
...
{
    echo "============================================================"
    echo " QMSum Topic-Representation Template Compare Summary"
    echo "============================================================"
