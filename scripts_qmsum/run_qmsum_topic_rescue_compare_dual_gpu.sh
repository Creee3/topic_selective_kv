#!/bin/bash
# ============================================================================
# Compare current top-1 topic routing vs a simple top-2 topic rescue.
#
# Fixed settings match the current best overnight baseline:
#   dynamic candidate pool
#   grounded prompt
#   answer max_new_tokens=96
#   qk_then_time selected evidence
#
# Usage:
#   cd ~/working_place/topic_selective_kv
#   START_DOC=0 END_DOC=30 MAX_QUERIES=5 GPU_A=0 GPU_B=1 \
#   bash scripts_qmsum/run_qmsum_topic_rescue_compare_dual_gpu.sh
# ============================================================================
...
LOG_ROOT=${LOG_ROOT:-logs/qmsum_topic_rescue_compare_${START_DOC}_${END_DOC}_q${MAX_QUERIES}}
...
case_tag_for_k() {
    local k="$1"
    echo "topicrescue_top${k}_dyn_grounded96_s${ROUTE_COARSE_SEGMENT_SIZE}_r${ROUTE_COARSE_SEGMENT_KEEP_RATIO}_m${ROUTE_COARSE_SEGMENT_MIN_KEEP}_k${ROUTE_TOP_K}_batch${QK_SCORE_BATCH_SIZE}_q${MAX_QUERIES}"
}
...
run_case() {
    ...
    local top_topics="$2"
    local tag
    tag="$(case_tag_for_k "$top_topics")"
    local log_dir="$LOG_ROOT/top${top_topics}"
...
append_summary() {
    ...
    if [ "$status" -ne 0 ] || [ ! -f "$case_tsv" ]; then
        {
            printf "%s\tRUN_FAIL\t%s" "$label" "$top_topics"
            for _field in \
                ...
                zero_recall_rate selected_zero_f1_rate avg_qk_candidates \
                avg_prefilter_prune avg_dynamic_pool_prune avg_qk_ms \
                avg_qk_total_ms avg_selected_ttft_ms ctx_token_saving_pct
            do
                printf "\tFAILED"
            done
            printf "\t%s\n" "$case_tsv"
    ...
    LABEL="$label" TOPICS="$top_topics" CASE_TSV="$case_tsv" python - <<'PY' >> "$SUMMARY_TSV"
import csv
import os

label = os.environ["LABEL"]
top_topics = os.environ["TOPICS"]
...
echo "============================================================"
echo " QMSum Topic Rescue Compare Dual GPU"
echo " docs=$START_DOC:$END_DOC MAX_QUERIES=$MAX_QUERIES"
echo " GPU_A=$GPU_A -> top1 current"
echo " GPU_B=$GPU_B -> top2 rescue"
...
cat > "$SUMMARY_TSV" <<EOF
case	status	hier_top_topics	n	avg_full_f1	avg_selected_f1	avg_oracle_f1	avg_selected_minus_full	avg_selected_minus_oracle	selected_bad_output_rate	avg_turn_recall	avg_turn_precision	avg_turn_f1	zero_recall_rate	selected_zero_f1_rate	avg_qk_candidates	avg_prefilter_prune	avg_dynamic_pool_prune	avg_qk_ms	avg_qk_total_ms	avg_selected_ttft_ms	ctx_token_saving_pct	case_tsv
EOF
append_summary "top1_current" 1 "$STATUS_A"
append_summary "top2_rescue" 2 "$STATUS_B"

{
    echo "============================================================"
    echo " QMSum Topic Rescue Compare Summary"
    echo "============================================================"
