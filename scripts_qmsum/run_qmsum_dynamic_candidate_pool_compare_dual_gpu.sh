#!/bin/bash
# ============================================================================
# Compare fixed exact-QK candidate pool vs query-type dynamic candidate pool.
#
# The routing and answer settings are otherwise kept identical:
...
#   GPU_A=0 GPU_B=1 \
#   bash scripts_qmsum/run_qmsum_dynamic_candidate_pool_compare_dual_gpu.sh
# ============================================================================
...
LOG_ROOT=${LOG_ROOT:-logs/qmsum_dynamic_candidate_pool_compare_dual_gpu}
...
case_tag_for_mode() {
    local mode="$1"
    if [ "$mode" = "fixed" ]; then
        echo "dynpool_fixed_grounded_s${ROUTE_COARSE_SEGMENT_SIZE}_r${ROUTE_COARSE_SEGMENT_KEEP_RATIO}_m${ROUTE_COARSE_SEGMENT_MIN_KEEP}_batch${QK_SCORE_BATCH_SIZE}_detail${DYNAMIC_DETAIL_TOP_K}_q${MAX_QUERIES}"
    else
        local clean_map
        clean_map="$(echo "$DYNAMIC_CANDIDATE_POOL_BUDGET_MAP" | tr ':,' '__')"
        echo "dynpool_${clean_map}_grounded_s${ROUTE_COARSE_SEGMENT_SIZE}_r${ROUTE_COARSE_SEGMENT_KEEP_RATIO}_m${ROUTE_COARSE_SEGMENT_MIN_KEEP}_batch${QK_SCORE_BATCH_SIZE}_detail${DYNAMIC_DETAIL_TOP_K}_q${MAX_QUERIES}"
    fi
}
...
run_case() {
    local gpu_id="$1"
    local mode="$2"
    local dynamic_pool_flag=0
    if [ "$mode" != "fixed" ]; then
        dynamic_pool_flag=1
    fi

    local tag
    tag="$(case_tag_for_mode "$mode")"
    ...
    DYNAMIC_CANDIDATE_POOL_BUDGET="$dynamic_pool_flag" \
...
append_summary() {
    ...
    if [ "$status" -ne 0 ] || [ ! -f "$case_tsv" ]; then
        echo "$mode	RUN_FAIL	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED	FAILED" >> "$SUMMARY_TSV"
        return
    fi

    MODE="$mode" CASE_TSV="$case_tsv" python - <<'PY' >> "$SUMMARY_TSV"
import csv
import os
...
echo "============================================================"
echo " QMSum Dynamic Candidate Pool Compare Dual GPU"
echo " docs=$START_DOC:$END_DOC MAX_QUERIES=$MAX_QUERIES"
echo " fixed GPU=$GPU_A"
echo " dynamic GPU=$GPU_B map=$DYNAMIC_CANDIDATE_POOL_BUDGET_MAP"
echo " answer_prompt_style=$ANSWER_PROMPT_STYLE"
echo " LOG_ROOT=$LOG_ROOT"
echo "============================================================"

run_case "$GPU_A" "fixed"
PID_A=$!
run_case "$GPU_B" "dynamic"
PID_B=$!
...
cat > "$SUMMARY_TSV" <<EOF
mode	status	avg_full_f1	avg_sel_f1	avg_oracle_f1	avg_delta	selected_bad_output_rate	avg_turn_recall	avg_qk_candidates	avg_dyn_pool_prune	avg_qk_ms	avg_qk_total_ms	avg_selected_ttft_ms	ctx_token_saving_pct
EOF
append_summary "fixed" "$STATUS_A"
append_summary "dynamic" "$STATUS_B"

{
    echo "============================================================"
    echo " QMSum Dynamic Candidate Pool Compare Summary"
    echo "============================================================"
